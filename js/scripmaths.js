...
var totalQuestions = questionsmaths.length;
...
function buildQuestion(){
    ...
    if(questionIndex > totalQuestions - 1){
        ...
        ans =  questionsmaths[questionIndex].answer;

        questionEl.innerHTML = questionsmaths[questionIndex].title;
        questionEl.setAttribute("class","text-left");
        questionEl.style.display= "block";

        for(var j = 0 ; j < 4 ; j++){
            var index = choiceArray[j].getAttribute("data-index");
            choiceArray[j].textContent = (+index+1) +". "+questionsmaths[questionIndex].choices[index];
