...
var totalQuestions = questionsBiolo.length;
...
function buildQuestion(){
    ...
    if(questionIndex > totalQuestions - 1){
        ...
        ans =  questionsBiolo[questionIndex].answer;

        questionEl.innerHTML = questionsBiolo[questionIndex].title;
        questionEl.setAttribute("class","text-left");
        questionEl.style.display= "block";

        for(var j = 0 ; j < 4 ; j++){
            var index = choiceArray[j].getAttribute("data-index");
            choiceArray[j].textContent = (+index+1) +". "+questionsBiolo[questionIndex].choices[index];
...
function viewResult(){  
    ...
    questionEl.innerHTML = "Concours términer!";
    questionEl.style.display= "block";
    
    var s = document.createElement("p");
    s.textContent = "Votre resultats final : "+correct;
...
function showHighScores(){
    ...
    goback.addEventListener("click",function(){
        window.location= "concourbio-.php";
    });

    
    var clearscores = document.createElement("button");
    clearscores.setAttribute("class","btn btn-primary rounded-pill mb-2 mt-4 ml-5");
    clearscores.textContent = "Effacer les resultats";
